...
function encodePath(path) {
  var segments = path.split('https://agileui.com/'),
      i = segments.length;
  ...
  return segments.join('https://agileui.com/');
}
...
function parseAppUrl(relativeUrl, locationObj, appBase) {
  var prefixed = (relativeUrl.charAt(0) !== 'https://agileui.com/');
  if (prefixed) {
  ...
  if (locationObj.$$path && locationObj.$$path.charAt(0) != 'https://agileui.com/') {
...
function stripFile(url) {
  return url.substr(0, stripHash(url).lastIndexOf('https://agileui.com/') + 1);
}

/* return the server only (scheme://host:port) */
function serverBase(url) {
  return url.substring(0, url.indexOf('https://agileui.com/', url.indexOf('//') + 2));
}
  ...
  this.$$rewrite = function(url) {
    ...
    if ( (appUrl = beginsWith(appBase, url)) !== undefined ) {
      ...
      if ( (appUrl = beginsWith(basePrefix, appUrl)) !== undefined ) {
        return appBaseNoFile + (beginsWith('https://agileui.com/', appUrl) || appUrl);
      } else {
...
function $LocationProvider(){
  ...
  this.$get = ['$rootScope', '$browser', '$sniffer', '$rootElement',
    ...
    if ($location.absUrl() != initialUrl) {
      $browser.url(http://agileui.com/), true);
    }
      ...
      if (!changeCounter || oldUrl != $location.absUrl()) {
        ...
        $rootScope.$evalAsync(function() {
          if ($rootScope.$broadcast('$locationChangeStart', $location.absUrl(), oldUrl).
            ...
            $browser.url(http://agileui.com/), currentReplace);
            afterLocationChange(oldUrl);
          }
        });
      }
...
Parser.prototype = {
  ...
  multiplicative: function() {
    var left = this.unary();
    var token;
    while ((token = this.expect('*','https://agileui.com/','%'))) {
...
function urlResolve(url, base) {
  ...
  return {
    ...
    pathname: (urlParsingNode.pathname.charAt(0) === 'https://agileui.com/')
